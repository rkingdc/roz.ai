...
def mock_genai():
    ...
    with patch('app.ai_services.genai', autospec=True) as mock_genai_lib:
        ...
        mock_response.parts = [MagicMock(text="Mocked AI Response")] # Ensure parts exist
...
def mock_tempfile():
    ...
    with patch('app.ai_services.tempfile.NamedTemporaryFile', return_value=mock_file) as mock_ntf, \
         patch('app.ai_services.os.remove') as mock_remove, \
         patch('app.ai_services.os.path.exists', return_value=True) as mock_exists: # Assume file exists for removal attempt
        yield {'mock_ntf': mock_ntf, 'mock_remove': mock_remove, 'mock_exists': mock_exists}
...
def test_generate_summary_text_file(app_context, mock_genai, mock_db):
    ...
    mock_genai.GenerativeModel.return_value.generate_content.return_value.text = "Text Summary"
...
def test_generate_summary_image_file(app_context, mock_genai, mock_db, mock_tempfile):
    ...
    mock_genai.GenerativeModel.return_value.generate_content.return_value.text = "Image Summary"
    ...
    mock_tempfile['mock_ntf']().__enter__().write.assert_called_once_with(b'fakeimagedata')
    ...
    mock_tempfile['mock_remove'].assert_called_once_with("/tmp/fake_temp_file_123") # Check temp file removed
...
def test_generate_summary_upload_error(app_context, mock_genai, mock_db, mock_tempfile):
    ...
    mock_tempfile['mock_remove'].assert_called_once_with("/tmp/fake_temp_file_123") # Ensure cleanup still happens
...
def test_generate_summary_api_error(app_context, mock_genai, mock_db):
    ...
    mock_genai.GenerativeModel.return_value.generate_content.side_effect = Exception("API Call Failed")
...
def test_generate_summary_api_blocked(app_context, mock_genai, mock_db):
    ...
    mock_genai.GenerativeModel.return_value.generate_content.side_effect = Exception("prompt was blocked due to safety")
...
def test_get_or_generate_summary_exists(app_context, mock_db, mock_genai):
    ...
    mock_db.get_file_details_from_db.assert_called_once_with(1) # Called without include_content
...
def test_get_or_generate_summary_generate_new(app_context, mock_db, mock_genai):
    ...
    mock_db.get_file_details_from_db.side_effect = [
        {
            'id': 1, 'filename': 'new.txt', 'mimetype': 'text/plain',
            'has_summary': False, 'summary': None
        },
        # Second call inside generate_summary (with content)
        {
            'id': 1, 'filename': 'new.txt', 'mimetype': 'text/plain',
            'content': b'Generate this.', 'has_summary': False, 'summary': None
        }
    ]
    mock_genai.GenerativeModel.return_value.generate_content.return_value.text = "Newly Generated Summary"
    ...
    mock_db.get_file_details_from_db.assert_has_calls([
        call(1),
        call(1, include_content=True)
    ])
    mock_genai.GenerativeModel.return_value.generate_content.assert_called_once() # Generation happened
    mock_db.save_summary_in_db.assert_called_once_with(1, "Newly Generated Summary") # Saved the new summary

def test_get_or_generate_summary_generate_new_save_fails(app_context, mock_db, mock_genai):
    """Test generating a new summary when saving it fails."""
    ai_services.gemini_configured = True
    mock_db.get_file_details_from_db.side_effect = [
        {'id': 1, 'filename': 'new.txt', 'mimetype': 'text/plain', 'has_summary': False, 'summary': None},
        {'id': 1, 'filename': 'new.txt', 'mimetype': 'text/plain', 'content': b'Generate this.', 'has_summary': False, 'summary': None}
    ]
    mock_genai.GenerativeModel.return_value.generate_content.return_value.text = "Generated But Not Saved"
    mock_db.save_summary_in_db.return_value = False # Simulate save failure

    result = ai_services.get_or_generate_summary(1)

    assert result == "Generated But Not Saved" # Should still return the generated summary
...
def test_generate_search_query_success(app_context, mock_genai):
    ...
    mock_genai.GenerativeModel.return_value.generate_content.return_value.text = " London weather forecast tomorrow " # With extra spaces

    result = ai_services.generate_search_query(user_message)

    assert result == "London weather forecast tomorrow" # Check cleaning
    mock_genai.GenerativeModel.assert_called_once_with('gemini-test-summary-model') # Uses summary model
...
def test_generate_search_query_cleaning(app_context, mock_genai):
    ...
    for i, messy_output in enumerate(test_cases):
        ...
        mock_genai.GenerativeModel.return_value.generate_content.return_value.text = messy_output
        result = ai_services.generate_search_query(user_message)
...
def test_generate_search_query_empty_response(app_context, mock_genai):
    ...
    ai_services.gemini_configured = True
    mock_genai.GenerativeModel.return_value.generate_content.return_value.text = "  " # Empty after strip
...
def create_mock_session_file(filename="session.txt", mimetype="text/plain", content=b"Session data"):
...
def test_generate_chat_response_basic(app_context, mock_genai, mock_db):
    ...
    mock_genai.GenerativeModel.return_value.generate_content.return_value.text = "Hello there!"
...
def test_generate_chat_response_with_calendar(app_context, mock_genai, mock_db):
    ...
    mock_db.get_chat_details_from_db.return_value = {'id': chat_id, 'model_name': 'gemini-test-default-model'}
    mock_db.get_chat_history_from_db.return_value = []
    mock_genai.GenerativeModel.return_value.generate_content.return_value.text = "You have a meeting."

    result = ai_services.generate_chat_response(chat_id, user_message, [], calendar_context, [])
...
def test_generate_chat_response_with_session_file(app_context, mock_genai, mock_db, mock_tempfile):
    """Test chat response with a session file."""
    ai_services.gemini_configured = True
    chat_id = 7
    user_message = "Summarize the session file."
    session_file_data = create_mock_session_file(filename="session_doc.txt", content=b"Session content")
    mock_db.get_chat_details_from_db.return_value = {'id': chat_id, 'model_name': 'gemini-test-default-model'}
    mock_db.get_chat_history_from_db.return_value = []
    mock_genai.GenerativeModel.return_value.generate_content.return_value.text = "Session Summary"
    mock_uploaded_file = MagicMock(uri="mock://session/uri")
    mock_genai.upload_file.return_value = mock_uploaded_file

    result = ai_services.generate_chat_response(chat_id, user_message, [], None, [session_file_data])

    assert result == "Session Summary"
    # Check temp file usage and upload
    mock_tempfile['mock_ntf'].assert_called_once()
    mock_tempfile['mock_ntf']().__enter__().write.assert_called_once_with(b'Session content')
    mock_genai.upload_file.assert_called_once_with(
        path="/tmp/fake_temp_file_123", display_name='session_session_doc.txt', mime_type='text/plain'
    )
    ...
    expected_gemini_context = [
        {'role': 'user', 'parts': [
            mock_uploaded_file, # Uploaded session file part
            user_message
        ]}
    ]
    assert generate_call.call_args[0][0] == expected_gemini_context
...
def test_generate_chat_response_with_permanent_file_full(app_context, mock_genai, mock_db, mock_tempfile):
    ...
    mock_db.get_file_details_from_db.return_value = { # Mock for the permanent file
        'id': file_id, 'filename': 'permanent.pdf', 'mimetype': 'application/pdf',
        'content': b'permanent pdf data', 'has_summary': False, 'summary': None
    }
    mock_genai.GenerativeModel.return_value.generate_content.return_value.text = "File Analysis"
    ...
    mock_tempfile['mock_ntf'].assert_called_once()
    mock_tempfile['mock_ntf']().__enter__().write.assert_called_once_with(b'permanent pdf data')
    mock_genai.upload_file.assert_called_once_with(
        path="/tmp/fake_temp_file_123", display_name='permanent.pdf', mime_type='application/pdf'
    )
    ...
    expected_gemini_context = [
        {'role': 'user', 'parts': [
            mock_uploaded_file, # Uploaded permanent file part
            user_message
        ]}
    ]
    assert generate_call.call_args[0][0] == expected_gemini_context
...
def test_generate_chat_response_with_web_search(app_context, mock_genai, mock_db, mock_web_search):
    ...
    result = ai_services.generate_chat_response(chat_id, user_message, [], None, [], enable_web_search=True)

    assert result == "Web Search Enhanced Response"
    # Check generate_search_query was called
    assert mock_genai.GenerativeModel.return_value.generate_content.call_count == 2 # Once for query, once for chat
    ...
    chat_generate_call = mock_genai.GenerativeModel.return_value.generate_content.call_args_list[1] # Second call
    expected_web_parts = [
...
def test_generate_chat_response_web_search_fails(app_context, mock_genai, mock_db, mock_web_search):
    ...
    mock_web_search.return_value = ["[System Error: Search API timed out]"] # Simulate error from search function
    ...
    chat_generate_call = mock_genai.GenerativeModel.return_value.generate_content.call_args_list[1]
    expected_gemini_context = [
        {'role': 'user', 'parts': [
...
def test_generate_chat_response_api_error(app_context, mock_genai, mock_db):
    ...
    mock_db.get_chat_details_from_db.return_value = {'id': chat_id, 'model_name': 'gemini-test-default-model'}
    mock_db.get_chat_history_from_db.return_value = []
    mock_genai.GenerativeModel.return_value.generate_content.side_effect = Exception("Chat API Failed")
    ...
    mock_db.add_message_to_db.assert_any_call(chat_id, 'assistant', "[Error communicating with AI: Chat API Failed]")
