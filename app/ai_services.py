...
from google.genai.types import GenerationConfig, Part, Content, SafetySetting, HarmCategory # Import necessary types
...
from pydantic_core import ValidationError # Import ValidationError for specific handling
import grpc
import logging
    ...
    try:
        ...
        # --- File Upload Logic ---
            ...
            try:
                ...
                content_parts = [Part(text=prompt), uploaded_file] # Wrap prompt, keep file object
                logger.info(
        ...
        logger.info(
            f"Calling generate_content with model '{summary_model_name}'."
        )
    ...
    while retries <= max_retries:
        ...
        try:
            ...
            if not response.parts:
                logger.warning(f"LLM response for query generation had no parts. Raw response: {response}")
...
def generate_chat_response(
    ...
    if web_search_enabled:
        ...
        if search_query:
            logger.info(f"Performing web search for chat {chat_id} with query: '{search_query}'")
            search_results = perform_web_search(search_query) # Assumes this returns a string or None
            if search_results:
                ...
                current_turn_parts.extend([
                 ...
                 current_turn_parts.append(Part(text="[System Note: Web search was performed but returned no results.]"))
        else:
            logger.info(f"Could not generate a suitable web search query for chat {chat_id}.")
            # MODIFIED: Wrap system note in Part()
    ...
    if user_message:
        ...
        if not current_turn_parts:
             logger.warning(f"Chat {chat_id}: User message is empty and no files/context were added.")
             # MODIFIED: Add a placeholder Part
             current_turn_parts.append(Part(text="[User provided no text, only attached files or context.]"))


    # --- Build Generation Config ---
    # Example: Configure safety settings if needed
    safety_settings = {
    ...
    generation_config = GenerationConfig(
        # Add other parameters like temperature, max_output_tokens if desired
        # temperature=0.7,
    ...
    try:
        logger.info(f"Sending request to Gemini for chat {chat_id} with {len(history)} history turns and {len(current_turn_parts)} current parts.")
        ...
        response = chat_session.send_message(
            content=Content(role="user", parts=current_turn_parts), # Pass the constructed list of Parts/Files
        ...
        # --- Process Response ---
        # Check for blocked prompt *before* accessing text (safety first)
        if response.prompt_feedback and response.prompt_feedback.block_reason:
            reason = response.prompt_feedback.block_reason
            logger.warning(f"User prompt blocked for chat {chat_id}. Reason: {reason}")
            assistant_reply = f"[AI Safety Error: Your request could not be processed due to safety settings (Reason: {reason}). Please modify your request.]"
        # Check for blocked response content
        elif response.candidates and response.candidates[0].finish_reason != 1: # 1 == FINISH_REASON_STOP
             reason_code = response.candidates[0].finish_reason
             reason_map = {
                 0: "Unspecified", 1: "Stop", 2: "Max Tokens",
                 3: "Safety", 4: "Recitation", 5: "Other"
             }
             reason_str = reason_map.get(reason_code, f"Unknown ({reason_code})")
             logger.warning(f"AI response finished unexpectedly for chat {chat_id}. Reason: {reason_str}")
             # Provide specific feedback for safety blocks
             if reason_code == 3: # Safety
                 safety_ratings_str = "N/A"
                 try:
                     # Extract safety ratings if available
                     ratings = response.candidates[0].safety_ratings
                     safety_ratings_str = ", ".join([f"{r.category.name}: {r.probability.name}" for r in ratings])
                 except Exception:
                     pass # Ignore errors extracting details
                 assistant_reply = f"[AI Safety Error: The generated response was blocked by safety filters. (Details: {safety_ratings_str}). Please try rephrasing or a different topic.]"
             elif reason_code == 2: # Max Tokens
                 assistant_reply = "[AI Error: The response exceeded the maximum length limit.]"
             else:
                 assistant_reply = f"[AI Error: Response generation failed or was stopped prematurely (Reason: {reason_str})]"

        # Check if parts exist before accessing text (should exist if not blocked)
        elif not response.parts:
            logger.error(f"Gemini response for chat {chat_id} had no parts but wasn't blocked. Raw Response: {response}")
            assistant_reply = "[AI Error: Received an empty response from the AI.]"
        else:
            # Success case
    ...
    # --- Specific Error Handling ---
        ...
        logger.error(f"Model '{model_to_use}' not found or inaccessible for chat {chat_id}: {e}")
        assistant_reply = f"[AI Error: Model '{raw_model_name}' not found or access denied.]"
    except GoogleAPIError as e:
        ...
        if "api key not valid" in err_str:
            ...
            try:
                ...
                elif response and response.candidates and response.candidates[0].finish_reason == 3:
            ...
            error_message = "[AI Error: API quota or rate limit exceeded. Please try again later.]"
             ...
             logger.error(f"Internal server error from Gemini API for chat {chat_id}: {e}")
...
def generate_text(prompt: str, model_name: str = None) -> str:
    ...
    try:
        ...
        # Basic safety/error checking (can be expanded like in chat)
        if response.prompt_feedback and response.prompt_feedback.block_reason:
            return f"[AI Safety Error: Prompt blocked ({response.prompt_feedback.block_reason})]"
        if response.candidates and response.candidates[0].finish_reason != 1: # STOP
             reason_code = response.candidates[0].finish_reason
             reason_map = { 2: "Max Tokens", 3: "Safety", 4: "Recitation", 5: "Other" }
             reason_str = reason_map.get(reason_code, f"Unknown ({reason_code})")
             return f"[AI Error: Generation stopped ({reason_str})]"
        if not response.parts:
