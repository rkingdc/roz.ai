...
# No longer need current_app here for configuration
...
def configure_gemini(app):
    ...
    # Access config directly from the passed app object
...
def generate_summary(file_id):
    ...
    """
    # Need access to config for SUMMARY_MODEL
    from flask import current_app # Okay to use here as it's called within request/command context

    if not gemini_configured: return "[Error: AI model not configured]"

    # Fetch file details including content BLOB
        ...
        if mimetype.startswith('text/'):
            ...
            try:
                ...
                print(f"File '{filename}' uploaded for summary, URI: {uploaded_file.uri}")
                 ...
                 print(f"Error preparing/uploading file '{filename}' for summary: {upload_err}")
                 return f"[Error preparing file for summary: {upload_err}]"
        else:
            print(f"Mimetype '{mimetype}' not directly supported for summary generation.")
...
def get_or_generate_summary(file_id):
    ...
    if file_details['has_summary'] and file_details['summary'] and not file_details['summary'].startswith("["):
        ...
        if database.save_summary_in_db(file_id, new_summary):
            ...
            print(f"Error: Failed to save newly generated summary for file ID: {file_id}")
            return new_summary

# --- Chat Response Generation ---

def generate_chat_response(chat_id, user_message, attached_files):
    """
    Generates a chat response using the appropriate model and context.
    Handles file uploads via API. Returns the assistant's reply string.
    """
    # Need access to config for DEFAULT_MODEL and TIMEOUT
    from flask import current_app
    ...
    if not chat_details:
        print(f"Error: Chat details not found for ID {chat_id} in generate_chat_response")
    ...
    try:
        if attached_files:
            ...
            for file_info in attached_files:
                ...
                file_details = database.get_file_details_from_db(file_id, include_content=True)
                if not file_details or 'content' not in file_details:
                    print(f"Warning: File details/content not found for ID {file_id}. Skipping.")
                ...
                try:
                    if attach_type == 'full':
                        print(f"Preparing file '{filename}' (Type: {mimetype}) for API upload...")
                        from werkzeug.utils import secure_filename
                        with tempfile.NamedTemporaryFile(delete=False, suffix=f"_{secure_filename(filename)}") as temp_file:
                            temp_file.write(content_blob)
                            temp_filepath = temp_file.name
