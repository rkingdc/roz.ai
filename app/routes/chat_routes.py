...
def send_message_route(chat_id):
    ...
    mode = data.get("mode", "chat") # Default to 'chat' if mode is not provided
    logger.info(f"Received message for chat {chat_id} with mode: '{mode}'")
    # -----------------------------------
    ...
    # For deep research, we primarily need the user message as the query.
    if mode == 'deep_research':
        if not user_message or user_message.isspace():
             return jsonify({"error": "Deep Research mode requires a text query."}), 400
        # In deep research mode, attached files, calendar context, and session files are ignored
        # as the deep_research function currently only takes the query string.
        # Web search is handled internally by deep_research.
        attached_files = []
        calendar_context = None
        session_files = []
        enable_web_search = False # Deep research handles its own search
        enable_streaming = False # Deep research returns a single report, not a stream
        logger.info(f"Deep Research mode selected. Ignoring files, context, web search flags.")
    ...
    # Always save the user message regardless of mode
    if user_message:
    ...
    try:
        assistant_response_content = None # Variable to hold the final response content string for saving

        if mode == 'deep_research':
            logger.info(f"Calling deep_research.perform_deep_research for chat {chat_id} with query: '{user_message[:50]}...'")
            # Call the deep research function
            report_string = deep_research.perform_deep_research(user_message)
            logger.info(f"deep_research.perform_deep_research returned (length: {len(report_string)}).")

            assistant_response_content = report_string # Store for saving

            # Return the report as JSON
            # Check if the report string indicates an error from deep_research
            status_code = 200
            error_prefixes = ("[Error:", "[CRITICAL Unexpected Error") # Check for error prefixes from deep_research
            is_error_reply = isinstance(report_string, str) and report_string.strip().startswith(error_prefixes)

            if is_error_reply:
                 logger.warning(f"Deep research report indicates an error for chat {chat_id}: {report_string}")
                 status_code = 500 # Indicate server error

            response_data = {"report": report_string}
            ...
            logger.info(f"Calling ai_services.generate_chat_response for chat {chat_id}. Streaming: {enable_streaming}")
            ai_response = ai_services.generate_chat_response(
                chat_id=chat_id,
            ...
            if enable_streaming:
                ...
                def stream_generator():
                    ...
                    try:
                        for chunk in ai_response: # Iterate the generator (which yields GenerateContentResponse chunks)
                            # Extract text or system message from chunk
                ...
                response = Response(stream_with_context(stream_generator()), mimetype='text/plain') # Or text/event-stream

            else:
                # If streaming is disabled, the ai_services function returns the full string or an error string directly
                ...
                assistant_response_content = assistant_reply # Store for saving
                ...
                response_data = {"reply": assistant_reply}
                response = jsonify(response_data)
                response.status_code = status_code # Set status code


        # --- Save the AI's response to the database (for non-streaming or after streaming completes) ---
        ...
        if not enable_streaming: # Only save here if not streaming (streaming saves in its generator's finally)
            ...
            if not final_content_to_save.strip(): # Ensure we don't save empty/whitespace
                 ...
                 logger.warning(f"Response for chat {chat_id} was empty. Saving placeholder.")

            logger.info(f"Attempting to save assistant message for chat {chat_id} (length: {len(final_content_to_save)}).")
            logger.debug(f"Calling db.add_message_to_db for assistant message (chat_id={chat_id}).") # Added log
            save_success = db.add_message_to_db(chat_id, "assistant", final_content_to_save)
            logger.debug(f"db.add_message_to_db for assistant message returned: {save_success}") # Added log
            if save_success:
                 logger.info(f"Successfully saved assistant message for chat {chat_id}. Save success: {save_success}")
            else:
                 logger.error(f"Failed to save assistant message for chat {chat_id}. Save success: {save_success}")
        ...
        # This needs to happen AFTER the message is saved to the DB.
        # For streaming, this might need to be triggered by the frontend after the stream finishes.
        # For non-streaming/deep_research, we can trigger it here.
        # Let's rely on the frontend to trigger loadSavedChats after receiving the final response/stream end.
        # This simplifies the backend response structure.

        # Clear temporary sidebar selection state after successful send (applies to both modes)
        # This should probably be handled on the frontend after the message is sent.
        # state.clearSidebarSelectedFiles(); # Moved to frontend

        # Do NOT clear attachedFiles state here. They persist per chat.
        # Only clear the session file state.
        # This should also be handled on the frontend after the message is sent.
        # if sentSessionFile and state.sessionFile === sentSessionFile: # sentSessionFile is not available here
        #      state.setSessionFile(null); # Moved to frontend


        return response # Return the prepared Flask Response object

    except Exception as e:
        # Catch unexpected errors *within this route handler* before calling ai_services/deep_research
        # or if ai_services/deep_research raises an exception not caught internally
        logger.error(
        ...
        # Use a separate try/except for saving the error message itself
        try:
            ...
            if save_success:
             ...
             logger.error(f"CRITICAL: Failed to save error message to DB for chat {chat_id}: {save_err}", exc_info=True)


        return (
            jsonify({"error": error_reply}), # Return the error message to the client
            500,
        )
