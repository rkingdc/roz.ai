...
def get_calendar_service():
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            print(f"ERROR: Missing or invalid {TOKEN_PATH}. Please run the Google auth flow manually first (e.g., using generate_token.py).")
    ...
    try:
        ...
        for event in events:
            ...
            try:
                ...
                start_formatted = start_dt.strftime('%Y-%m-%d %H:%M') # UTC time for consistency
            except ValueError:
        ...
        output_str = f"Upcoming Google Calendar Events (Next {months} Months):\n" + "\n".join(event_list_items)
        return output_str
