...
This project includes a Makefile to simplify common tasks. Here's how to use it:

1.  **Using the Makefile:** The Makefile provides commands for starting, stopping, and testing the application. It uses `uvicorn` to run the application and `pytest` to run tests. To use these commands, navigate to the project root directory in your terminal.
    ...
    *   **Start the application:**
        ...
        This command stops any existing instances of the application, then starts a new instance, directing logs to both the terminal (stdout) and a file named `roz.ai.log` in the project root.
    ...
    *   **Stop the application:**
        ...
        This command stops the application by searching for its process and terminating it. It uses `pkill`, so ensure that command is available on your system.
    ...
    *   **Run tests:**
        ...
        This command executes the unit tests using `pytest`. Ensure that `pytest` is installed within your virtual environment.
...
2.  **Manual Execution (Alternative):**  If you prefer, you can run the Flask development server directly.  Activate the virtual environment and run:

    ```bash
    flask --app run run --host=0.0.0.0 --port=5678
    ```
...
3.  **Access the application in your web browser:**
    ...
    *   Open your web browser and navigate to `http://127.0.0.1:5678` (or the address shown in the terminal). If you used the Makefile, the default port is 8000.
